...
fig, axes = plt.subplots(2, 1, figsize=(14, 8.6))

# ---- (a) speed + streamlines ----
ax = axes[0]
pc = ax.pcolormesh(x_mm, y_mm, np.ma.masked_invalid(speedT), cmap="turbo",
                   shading="auto", vmin=0, vmax=min(gap_peak, 7.0))
ax.streamplot(x_mm, y_mm, uxT, uyT, color="k", density=2.2, linewidth=0.6,
...
ann(ax, "高速射流\n(可变环隙)", (6.5, 6.6), (14, 12), "w")
ann(ax, "锐边:分离点固定", (6.0, -6.0), (18, -13), "yellow")
ann(ax, "回流区 / 尾涡", (28, 3.2), (40, 11), "w")
